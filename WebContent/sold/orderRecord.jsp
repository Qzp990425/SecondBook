            ...
            <div class="layui-field-box">
              ...
              <table class="layui-table">
                  ...
                  <tbody>
                      ...
                      <tr>
                        <td>${order.uuid}</td>
                        <td><img src="${pageContext.request.contextPath}/bookImg/${order.book.uuid}.jpg" /></td>
                        <td>${order.book.name}</td>
                        <td>${order.book.price}</td>
                        <td>${order.book.nowPrice}</td>
                        <td>${order.number}</td>
                        <th>${order.name}</th>
                        ...
                        <td>
                        	...
                        	<a href="${pageContext.request.contextPath}/orderRecord/sendOrder?oid=${order.oid}" class="layui-btn layui-btn-normal">发货</a>
                        	</c:if>
                        </td>
                    </tr>      
                    </c:forEach>             
                  	  ...
                  	  <tr style="text-align:center">
                  	  	<td colspan="10">
         	  			   <c:if test="${pageInfo.prePage!=0}">
                               <a href="${pageContext.request.contextPath}/orderRecord/getAllOrder?pageNum=${pageInfo.prePage}" class="layui-btn layui-btn-normal">上一页</a>
                           </c:if>
                           <c:forEach begin="1" end="${pageInfo.pages}" varStatus="pageNum">
                               <a href="${pageContext.request.contextPath}/orderRecord/getAllOrder?pageNum=${pageNum.index}" class="layui-btn layui-btn-normal">${pageNum.index}</a>							
					      </c:forEach>
					      <c:if test="${pageInfo.nextPage!=0}">
						       <a href="${pageContext.request.contextPath}/orderRecord/getAllOrder?pageNum=${pageInfo.nextPage}"  class="layui-btn layui-btn-normal">下一页</a>			
                          </c:if>
                  	  	</td>
                  	  </tr>
              </table>
