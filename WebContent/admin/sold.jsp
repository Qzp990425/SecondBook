        ...
        <fieldset class="layui-elem-field">
            ...
            <div class="layui-field-box">
              ...
              <table class="layui-table">
                  ...
                  <tbody>
                  	<c:forEach items="${pageInfo.list}" var="sold">
                      <tr>                     
                      	<td>${sold.uid}</td>
                      	<td>${sold.tel}</td>
                      	<td>${sold.sno}</td>
                  	  ...
                  	  <tr style="text-align:center">
                  	  	<td colspan="4">
         	  			   <c:if test="${pageInfo.prePage!=0}">
                               <a href="${pageContext.request.contextPath}/user/getAllUserOfSold?pageNum=${pageInfo.prePage}" class="layui-btn layui-btn-normal">上一页</a>
                           </c:if>
                           <c:forEach begin="1" end="${pageInfo.pages}" varStatus="pageNum">
                               <a href="${pageContext.request.contextPath}/user/getAllUserOfSold?pageNum=${pageNum.index}" class="layui-btn layui-btn-normal">${pageNum.index}</a>							
					      </c:forEach>
					      <c:if test="${pageInfo.nextPage!=0}">
						       <a href="${pageContext.request.contextPath}/user/getAllUserOfSold?pageNum=${pageInfo.nextPage}"  class="layui-btn layui-btn-normal">下一页</a>			
                          </c:if>
                  	  	</td>
                  	  </tr>
              </table>
