                  	  	...
                  	  	<td colspan="10">
                  	  		<c:if test="${type==null}">
	         	  			   <c:if test="${pageInfo.prePage!=0}">
	                               <a href="${pageContext.request.contextPath}/book/getAllBook?pageNum=${pageInfo.prePage}" class="layui-btn layui-btn-normal">上一页</a>
	                           </c:if>
	                           <c:forEach begin="1" end="${pageInfo.pages}" varStatus="pageNum">
	                               <a href="${pageContext.request.contextPath}/book/getAllBook?pageNum=${pageNum.index}" class="layui-btn layui-btn-normal">${pageNum.index}</a>							
						      </c:forEach>
						      <c:if test="${pageInfo.nextPage!=0}">
							       <a href="${pageContext.request.contextPath}/book/getAllBook?pageNum=${pageInfo.nextPage}"  class="layui-btn layui-btn-normal">下一页</a>			
	                          </c:if>
                          </c:if>
                           <c:if test="${type!=null}">
	         	  			   <c:if test="${pageInfo.prePage!=0}">
	                               <a href="${pageContext.request.contextPath}/book/getAllBookByType?pageNum=${pageInfo.prePage}&type=${type}" class="layui-btn layui-btn-normal">上一页</a>
	                           </c:if>
	                           <c:forEach begin="1" end="${pageInfo.pages}" varStatus="pageNum">
	                               <a href="${pageContext.request.contextPath}/book/getAllBookByType?pageNum=${pageNum.index}&type=${type}" class="layui-btn layui-btn-normal">${pageNum.index}</a>							
						      </c:forEach>
						      <c:if test="${pageInfo.nextPage!=0}">
							       <a href="${pageContext.request.contextPath}/book/getAllBookByType?pageNum=${pageInfo.nextPage}&type=${type}"  class="layui-btn layui-btn-normal">下一页</a>			
	                          </c:if>
                          </c:if>
                  	  	</td>
                  	  </tr>
